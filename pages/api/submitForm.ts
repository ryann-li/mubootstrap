...
async function NotifyDiscord(form: { [key: string]: string }, whurl: string) {
  ...
  console.log('Sending to Discord webhook:', whurl, webhookBody);
  ...
  if (!response.ok) {
    console.error('Failed to notify Discord:', response.status, response.statusText);
    throw new Error('Failed to notify Discord');
  }
...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  console.log('Webhook identifier:', webhookIdentifier);
  console.log('Resolved webhook URL:', whurl);

  if (!whurl) {
    console.error('Invalid webhook identifier:', webhookIdentifier);
    return res.status(400).json({ error: "Invalid webhook identifier" });
  }

  try {
    console.log('Submitting form data to Google Apps Script:', form);
    const response = await fetch(new_app_script_url, {
      method: 'POST',
      body: new URLSearchParams({
        ...form,
